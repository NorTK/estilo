...
## 1. Realismo Técnico y Fidelidad Lingüística
...
* **Rechazo tajante a la ingeniería lingüística ideológica**:
  * Prohibido el uso de desdoblamientos artificiales, lenguaje presuntamente inclusivo con morfemas inventados o circunloquios forzados que degraden la claridad y velocidad de lectura del operador.
...
* **Preservación incondicional de la terminología técnica canónica**:
  * Prohibida la censura o sustitución eufemística de términos estándar. Mantener de forma estricta términos técnicos consolidados como `master`/`slave`, `whitelist`/`blacklist`, `abort`, `kill` o `parent`/`child` donde las interfaces del kernel Linux, protocolos de red, buses de hardware o archivos de configuración así lo requieran.
  * Casos concretos: En agregación de enlaces, zonas de replicación DNS con BIND, replicación de PostgreSQL, buses SPI/I2C o controladores de hardware, se utiliza la nomenclatura técnica exacta del subsistema.
...
* **Español técnico de alta fidelidad sin anglicismos innecesarios**:
  * Prohibido intercalar términos en inglés cuando exista un término técnico consolidado en español. Prohibido escribir construcciones como trampas operativas entre paréntesis, ensamblajes con su traducción al inglés o aclaraciones redundantes.
  * Preservar exclusivamente la terminología canónica de la industria cuando sea estándar de facto en consolas y llamadas al sistema: *socket*, *cluster*, *kernel*, *bare-metal*, *daemon*, *pipeline*, *pool* y *firmware*.
...
* **Reglas de capitalización y ortografía técnica**:
  * **Prohibición estricta de mayúsculas sostenidas**: Ningún título, encabezado, sección, etiqueta de menú o título de admonición debe escribirse en mayúsculas sostenidas. En español, escribir palabras completas en mayúsculas representa una falta ortográfica, entorpece la lectura y satura visualmente la interfaz.
  * **Prohibición del estilo de titular en inglés**: En español, los títulos y subtítulos llevan mayúscula únicamente en la letra inicial y en los nombres propios. Queda prohibido el estilo anglosajón en el cual cada palabra comienza con mayúscula.
...
* **Prohibición de paréntesis en títulos y erradicación en el contenido**:
  * Ningún título o encabezado debe contener paréntesis. Los paréntesis en títulos representan ruido visual y denotan falta de concisión sintáctica.
  * En el cuerpo del texto, erradicar los paréntesis innecesarios. No usar paréntesis como muleta para sobre-explicar, traducir o intercalar aclaraciones obvias. Si una precisión técnica es indispensable, redáctela de forma directa en la oración; si no lo es, descártela.
...
* **Prohibición absoluta en salidas textuales**:
  ...
  * Razón técnica: Las imágenes de texto degradan la accesibilidad, impiden la copia y pegado determinista, no son indexables por motores de búsqueda y complican el mantenimiento evolutivo. Todo contenido textual debe residir en bloques `.. code-block::`.

* **Encuadre, dimensiones y legibilidad**:
  * **Encuadre estricto**: Recortar la imagen exactamente a la ventana activa o control relevante. Prohibido capturar pantallas completas con escritorios vacíos, fondos de pantalla o barras del sistema operativo anfitrión.
  * **Resolución máxima**: No superar 1920x1080 píxeles. Utilizar la resolución mínima necesaria que garantice total nitidez (dimensiones recomendadas entre 800x600 y 1280x720 píxeles).
  * **Tamaño tipográfico**: Incrementar la escala de visualización o tamaño de fuente en la interfaz gráfica antes de la captura, asegurando que el texto sea legible a un tamaño de al menos 14 a 16 píxeles sin necesidad de ampliar la imagen.
  * **Tema visual**: Utilizar temas visuales oscuros o de alto contraste conformes con la estética sobria de NorTK.

* **Formato y optimización**:
  * Formato rasterizado: PNG con compresión sin pérdidas, procesado previamente con herramientas de optimización como `optipng`.
  * Formato vectorial: SVG para diagramas, esquemas y logotipos.
  * Almacenamiento: Directorio `source/_static/images/` con nombres descriptivos en minúsculas y guiones (`kebab-case.png`).

* **Inclusión en reStructuredText y accesibilidad acústica**:
  ...
  * Usar `:align: center` y ancho porcentual adaptable (`:width: 100%` o acotado con `:width: 85%`). Prohibido definir alturas fijas en píxeles.
...
* **Corrección automática de formato con crstlint**:
  * Para corregir automáticamente omisiones comunes de marcado (como líneas en blanco previas a listas), ejecute localmente:
    ```bash
    make fix
    ```
    (invoca `crstlint -fr .`).
...
* **Hook pre-push de Git**:
  * El repositorio cuenta con un hook en `.githooks/pre-push` configurado mediante `make hooks` (`git config core.hooksPath .githooks`). Este hook ejecuta automáticamente `rstcheck -r .` antes de cualquier envío remoto, abortando la operación si detecta advertencias o errores.

* **Compilación estricta**:
  * La compilación en `GNUmakefile` utiliza `SPHINXOPTS ?= -W --keep-going`, convirtiendo cualquier advertencia en error fatal.

* **Flujo de desarrollo en vivo**:
  * Utilizar `make dev` para arrancar el servidor `sphinx-autobuild` con el constructor `dirhtml` en `http://127.0.0.1:8000/`.

* **Memoria del proyecto**:
  * Toda decisión arquitectónica, trampa descubierta o problema resuelto debe registrarse en el diario de proyecto mediante `ajourn log -m "<mensaje>" -t "<TAG>"`.
